<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        <h2 class="font-headline-md text-headline-md text-on-surface">Bonafide Certificate Management</h2>
        <p class="text-body-md font-body-md text-on-surface-variant mt-1">Official certification confirming active student enrollment, date of birth, and class details.</p>
      </div>
      <div class="flex items-center gap-2">
        <a href="<?php echo site_url('certificates/generate'); ?>" class="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-lg bg-secondary text-on-secondary text-label-md font-semibold hover:bg-on-secondary-fixed-variant transition-colors shadow-sm">
          <span class="material-symbols-outlined text-[18px]">add_circle</span>Generate Bonafide
        </a>
      </div>
    </div>

    <!-- Bonafide Records Table -->
    <div class="elevation-1 rounded-xl bg-surface-container-lowest border border-outline-variant/50 overflow-hidden mb-6">
      <div class="p-4 border-b border-outline-variant/50 flex items-center justify-between">
        <span class="text-body-md font-semibold text-on-surface">Issued Bonafide Certificates (<?php echo count($certificates); ?>)</span>
      </div>
        ...
        <table class="w-full data-table zebra border-collapse text-body-md">
          ...
          <tbody class="divide-y divide-outline-variant/40">
            <?php if (empty($certificates)): ?>
              <tr><td colspan="8" class="px-4 py-8 text-center text-on-surface-variant">No bonafide certificates issued yet.</td></tr>
            <?php else: ?>
              <?php foreach ($certificates as $c): ?>
                <tr class="hover:bg-surface-container-low transition-colors">
                  ...
                  <td class="px-4 py-3 text-[12px] text-on-surface-variant max-w-[200px] truncate"><?php echo html_escape($c->remarks ?: 'Official Verification'); ?></td>
