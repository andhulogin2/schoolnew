<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        <h2 class="font-headline-md text-headline-md text-on-surface">Study & Enrollment Certificates</h2>
        <p class="text-body-md font-body-md text-on-surface-variant mt-1">Certify academic pursuit, enrollment periods, course completion, and institutional duration.</p>
      </div>
      <div class="flex items-center gap-2">
        <a href="<?php echo site_url('certificates/generate'); ?>" class="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-lg bg-secondary text-on-secondary text-label-md font-semibold hover:bg-on-secondary-fixed-variant transition-colors shadow-sm">
          <span class="material-symbols-outlined text-[18px]">add_circle</span>Generate Study Cert
        </a>
      </div>
    </div>
    ...
    <div class="elevation-1 rounded-xl bg-surface-container-lowest border border-outline-variant/50 overflow-hidden mb-6">
      <div class="p-4 border-b border-outline-variant/50 flex items-center justify-between">
        <span class="text-body-md font-semibold text-on-surface">Issued Study Certificates (<?php echo count($certificates); ?>)</span>
      </div>
        ...
        <table class="w-full data-table zebra border-collapse text-body-md">
          <thead>
            <tr class="border-b border-outline-variant/60 bg-surface-container-low/50">
              ...
              <th class="text-left px-4 py-3 text-label-md font-semibold text-on-surface-variant uppercase whitespace-nowrap">Academic Session</th>
          ...
          <tbody class="divide-y divide-outline-variant/40">
            <?php if (empty($certificates)): ?>
              <tr><td colspan="7" class="px-4 py-8 text-center text-on-surface-variant">No study certificates generated yet.</td></tr>
            <?php else: ?>
              <?php foreach ($certificates as $c): ?>
                <tr class="hover:bg-surface-container-low transition-colors">
                  ...
                  <td class="px-4 py-3 whitespace-nowrap text-[13px] text-on-surface font-medium"><?php echo html_escape($c->year_name); ?></td>
