<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        <h2 class="font-headline-md text-headline-md text-on-surface">WhatsApp Templates Management</h2>
        <p class="text-body-md font-body-md text-on-surface-variant mt-1">Provider-independent WhatsApp messaging templates categorized by Attendance, Fees, Exam, and Transport.</p>
      </div>
    ...
    <div class="elevation-1 rounded-xl bg-surface-container-lowest border border-outline-variant/50 overflow-hidden mb-6">
      <div class="p-4 border-b border-outline-variant/50 flex items-center justify-between">
        <span class="text-body-md font-semibold text-on-surface">WhatsApp Templates (<?php echo count($templates); ?>)</span>
      </div>
          ...
          <tbody class="divide-y divide-outline-variant/40">
            <?php foreach ($templates as $tmpl): ?>
              <tr class="hover:bg-surface-container-low transition-colors">
                ...
                <td class="px-4 py-3 whitespace-nowrap font-mono font-bold text-emerald-600 text-[12px]">
                  <?php echo html_escape($tmpl->template_code); ?>
                </td>
                <td class="px-4 py-3 whitespace-nowrap text-[13px] text-on-surface font-medium">
                  <?php echo html_escape($tmpl->category); ?>
                </td>
                ...
                <td class="px-4 py-3 text-center whitespace-nowrap">
                  <div class="flex items-center justify-center gap-1.5">
                    <button onclick="previewWhatsApp('<?php echo html_escape(addslashes($tmpl->template_name)); ?>', '<?php echo html_escape(addslashes($tmpl->message_template)); ?>')" class="p-1 rounded hover:bg-surface-container-high text-primary font-semibold text-xs inline-flex items-center gap-1 cursor-pointer">
    ...
    <!-- WhatsApp Preview Modal Dialog -->
    <dialog id="waPrevModal" class="p-0 rounded-2xl bg-surface-container-lowest border border-outline-variant/50 elevation-3 w-full max-w-md backdrop:bg-scrim/40">
      <div class="p-6 space-y-4">
        <div class="flex items-center justify-between pb-3 border-b border-outline-variant/50">
          <h3 id="waPrevTitle" class="font-headline-md text-title-md font-bold text-on-surface">WhatsApp Message Preview</h3>
          <button onclick="document.getElementById('waPrevModal').close()" class="p-1 rounded-full hover:bg-surface-container-high text-on-surface-variant">
        ...
        <div class="flex items-center justify-end pt-3 border-t border-outline-variant/50">
          <button type="button" onclick="document.getElementById('waPrevModal').close()" class="px-4 py-2 rounded-lg bg-emerald-600 text-white text-label-md font-semibold hover:bg-emerald-700">Close</button>
        </div>
      </div>
    </dialog>

    <script>
      function previewWhatsApp(name, text) {
        document.getElementById('waPrevTitle').innerText = name;
        ...
        .then(res => res.json())
        .then(data => {
          document.getElementById('waRenderedText').innerText = data.compiled;
          document.getElementById('waPrevModal').showModal();
        });
      }
    </script>
