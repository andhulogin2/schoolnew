<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-body-md font-body-md text-on-surface-variant mt-1">Review teacher leave applications, casual/earned leave requests, and track departmental staffing coverage.</p>
      </div>
    ...
    <div class="elevation-1 rounded-xl bg-surface-container-lowest border border-outline-variant/50 p-4 mb-6">
      <form method="get" action="<?php echo site_url('leave/staff_leave'); ?>" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
          <label class="block font-label-md text-label-md text-on-surface mb-1 font-medium">Department</label>
          <select name="department_id" onchange="this.form.submit()" class="w-full px-3 py-2 rounded-lg border border-outline-variant bg-surface-container-lowest text-body-md focus:ring-2 focus:ring-primary/20 focus:border-primary">
            <option value="">All Departments</option>
            <?php foreach ($departments as $d): ?>
              <option value="<?php echo $d->department_id; ?>" <?php echo (($filters['department_id'] ?? '') == $d->department_id) ? 'selected' : ''; ?>><?php echo html_escape($d->department_name); ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        ...
        <div>
          ...
          <div class="flex gap-2">
            <input type="text" name="search" value="<?php echo html_escape($filters['search'] ?? ''); ?>" placeholder="Staff name, employee code..." class="w-full px-3 py-2 rounded-lg border border-outline-variant bg-surface-container-lowest text-body-md focus:ring-2 focus:ring-primary/20 focus:border-primary"/>
    ...
    <div class="elevation-1 rounded-xl bg-surface-container-lowest border border-outline-variant/50 overflow-hidden mb-6">
      <div class="p-4 border-b border-outline-variant/50 flex items-center justify-between">
        <span class="text-body-md font-semibold text-on-surface">Staff Applications (<?php echo count($applications); ?>)</span>
      </div>
        ...
        <table class="w-full data-table zebra border-collapse text-body-md">
          <thead>
            <tr class="border-b border-outline-variant/60 bg-surface-container-low/50">
              ...
              <th class="text-left px-4 py-3 text-label-md font-semibold text-on-surface-variant uppercase whitespace-nowrap">Department</th>
          ...
          <tbody class="divide-y divide-outline-variant/40">
            <?php if (empty($applications)): ?>
              <tr><td colspan="8" class="px-4 py-8 text-center text-on-surface-variant">No staff leave applications found.</td></tr>
            <?php else: ?>
              <?php foreach ($applications as $a): ?>
                ...
                <tr class="hover:bg-surface-container-low transition-colors">
                  <td class="px-4 py-3 whitespace-nowrap">
                    <strong class="text-on-surface block"><?php echo html_escape($a->staff_name); ?></strong>
                    <span class="text-[11px] text-on-surface-variant font-mono"><?php echo html_escape($a->employee_code); ?></span>
                  </td>
                  <td class="px-4 py-3 whitespace-nowrap font-medium text-on-surface text-[13px]">
                    <?php echo html_escape($a->department_name ?: 'General Faculty'); ?>
                  </td>
                  <td class="px-4 py-3 whitespace-nowrap">
