<?php defined('BASEPATH') OR exit('No direct script access allowed'); ?>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        <h2 class="font-headline-md text-headline-md text-on-surface">Student Accounts</h2>
        <p class="text-body-md font-body-md text-on-surface-variant mt-1">Student portal login accounts mapped directly to student records.</p>
      </div>
      <div class="flex items-center gap-2">
        <a href="<?php echo site_url('users/create'); ?>" class="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-lg bg-secondary text-on-secondary text-label-md font-semibold hover:bg-on-secondary-fixed-variant transition-colors shadow-sm">
          <span class="material-symbols-outlined text-[18px]">person_add</span>Add Student User
        </a>
      </div>
    </div>
    ...
    <div class="elevation-1 rounded-xl bg-surface-container-lowest border border-outline-variant/50 overflow-hidden mb-6">
      <div class="p-4 border-b border-outline-variant/50 flex items-center justify-between">
        <span class="text-body-md font-semibold text-on-surface">Registered Student Accounts (<?php echo count($users); ?>)</span>
      </div>
        ...
        <table class="w-full data-table zebra border-collapse text-body-md">
          <thead>
            <tr class="border-b border-outline-variant/60 bg-surface-container-low/50">
              ...
              <th class="text-left px-4 py-3 text-label-md font-semibold text-on-surface-variant uppercase whitespace-nowrap">Email</th>
          ...
          <tbody class="divide-y divide-outline-variant/40">
            <?php if (empty($users)): ?>
              <tr><td colspan="6" class="px-4 py-8 text-center text-on-surface-variant">No student accounts registered.</td></tr>
            <?php else: ?>
              <?php foreach ($users as $u): ?>
                <tr class="hover:bg-surface-container-low transition-colors">
                  ...
                  <td class="px-4 py-3 whitespace-nowrap font-mono text-xs text-on-surface"><?php echo html_escape($u->admission_number ?: 'N/A'); ?></td>
